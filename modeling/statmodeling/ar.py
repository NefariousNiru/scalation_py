        ...
        for i in tqdm(range(self.n_features)):
            for j in tqdm(range(self.test_size - sample_offset)):
                if j % self.rc == 0:
                    if self.skip_insample is None:
                        ...
                        train_data = self.data.iloc[
                            :, i
                        ]  # use entire data for in-sample.

                    if self.args["ar_type"] == "OLS":
                        ar_model = AutoReg(
                            train_data, lags=self.p, trend=self.trend, seasonal=False
                        ).fit()
                    elif self.args["ar_type"] == "yule_walker_adjusted":
                        rho, sigma = yule_walker(
                            train_data, order=self.p, method="adjusted"
                        )
                        intercept = (
                            np.mean(train_data) * (1 - np.sum(rho))
                            if self.trend == "c"
                            else 0
                        )
                    ...
                    if self.skip_insample is None:
                        new_data = self.data.iloc[
                            self.train_size + j - 1 : self.train_size + j, i
                        ]
                        train_data = pd.concat([train_data, new_data])

                        if self.args["ar_type"] == "OLS":
                            ar_model = ar_model.append(endog=new_data, refit=False)
                        elif self.args["ar_type"] in [
                            "yule_walker_adjusted",
                            "yule_walker_mle",
                        ]:
                            rho, sigma = yule_walker(
                                train_data,
                                order=self.p,
                                method=self.args["ar_type"].split("_")[2],
                            )
                            intercept = (
                                np.mean(train_data) * (1 - np.sum(rho))
                                if self.trend == "c"
                ...
                if self.args["ar_type"] == "OLS":
                    ...
                    model = AutoReg(train_data, lags=self.p, trend=self.trend)
                    forecasts = model.predict(
                        params=params, start=start, end=end, dynamic=True
                    ).values.T.reshape(1, self.pred_len)
