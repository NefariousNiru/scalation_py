...
class ARIMA(Model):
    ...
    def train_test(self) -> None:
        ...
        for i in tqdm(range(self.n_features)):
            for j in tqdm(range(self.test_size - sample_offset)):
                if j % self.rc == 0:
                    ...
                    arima_model = ARIMA_(
                        train_data, order=(self.p, self.d, self.q), trend=self.trend
                    ).fit(method=self.fit_method)
                else:
                    if self.skip_insample is None:
        ...
        self.total_params = len(
            arima_model.params.index[arima_model.params.index != "sigma2"]
        )
