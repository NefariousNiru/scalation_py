...
class TrendBasis(nn.Module):
    def __init__(
        ...
        self.p = p
        backcast_grid = np.stack(
            [np.power(np.arange(backcast_size) / backcast_size, i) for i in range(p)]
        )
        forecast_grid = np.stack(
            [np.power(np.arange(forecast_size) / forecast_size, i) for i in range(p)]
        )
        self.backcast_basis = nn.Parameter(
            t.tensor(backcast_grid, dtype=t.float32), requires_grad=False
        )
        self.forecast_basis = nn.Parameter(
            t.tensor(forecast_grid, dtype=t.float32), requires_grad=False
        )
    ...
    def __init__(self, harmonics: int, backcast_size: int, forecast_size: int):
        ...
        def fourier(time, f):
            return np.concatenate(
                [np.cos(2 * np.pi * f * time), np.sin(2 * np.pi * f * time)], axis=0
            )
        ...
        self.backcast_template = nn.Parameter(
            t.tensor(
                np.stack([fourier(t_b, f) for f in freq], axis=0).reshape(
                    2 * harmonics, backcast_size
                ),
                dtype=t.float32,
            ),
            requires_grad=False,
        ...
        self.forecast_template = nn.Parameter(
            t.tensor(
                np.stack([fourier(t_f, f) for f in freq], axis=0).reshape(
                    2 * harmonics, forecast_size
                ),
                dtype=t.float32,
            ),
            requires_grad=False,
    ...
    def __init__(self, configs):
            ...
            def make_block():
                return NBeatsBlock(
                    ...
                    basis_function=TrendBasis(
                        configs.degree_of_polynomial, self.seq_len, self.pred_len
                    ),
            ...
            def make_block():
                return NBeatsBlock(
                    ...
                    basis_function=SeasonalityBasis(
                        configs.harmonics, self.seq_len, self.pred_len
                    ),
                    layers=configs.n_layers,
                    layer_size=configs.layer_size,
                )
        ...
        if self.individual:
            self.models = nn.ModuleList(
                [
                    NBeats(
                        nn.ModuleList([make_block() for _ in range(configs.n_blocks)])
                    )
