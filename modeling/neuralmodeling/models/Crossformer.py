...
from modeling.neuralmodeling.layers.attn import (
    FullAttention,
    AttentionLayer,
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.enc_pos_embedding = nn.Parameter(
            torch.randn(
                1, self.data_dim, (self.pad_in_len // self.seg_len), self.d_model
            )
        )
        ...
        self.dec_pos_embedding = nn.Parameter(
            torch.randn(
                1, self.data_dim, (self.pad_out_len // self.seg_len), self.d_model
            )
        )
