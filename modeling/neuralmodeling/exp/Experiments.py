...
class Experiment(object):
    def __init__(self, args):
        if args is not None:
            ...
            if self.qof_calculation_mode not in [
                "single_horizon",
                "aggregated_horizons",
            ]:
                raise ValueError(
    ...
    def train(self, setting) -> None:
        ...
        early_stopping = EarlyStopping(
            patience=self.args["patience"], verbose=self.debugging
        )
    ...
    def test(self, setting) -> None:
        ...
        with torch.no_grad():
            ...
            for i, (
                ...
                if self.features == "m":
                    ...
                    batch_y = (
                        batch_y[:, -self.pred_len :, -1].unsqueeze(-1).to(self.device)
                    )
        ...
        self.forecast_tensor = np.full(
            shape=(preds.shape[0], self.pred_len, self.args["c_out"]), fill_value=np.nan
        )
        ...
        self.forecast_tensor_original = test_data.inverse_transform(
            self.forecast_tensor.reshape(shape[0] * shape[1], -1)
        ).reshape(shape)
