...
def train_test_split(
    x: np.ndarray, y: np.ndarray, test_ratio: float = 0.25
...
def make_input_output_sequences(series, n_past, n_future, include_dates):
    ...
    if include_dates is True:
        ...
        return torch.from_numpy(np.array(x[:, :, 1:], dtype=np.float32)).float().to(
            device
        ), torch.from_numpy(np.array(y[:, :, 1:], dtype=np.float32)).float().to(device)
...
def train_test_split(
    data: pd.DataFrame, train_ratio: float = 0.8, val_ratio: float = 0
