...
class Dataset(Dataset):
    def __init__(
        ...
        self.target = target
        self.normalization = experiment.normalization
        self.timeenc = timeenc
        self.freq = freq

        self.experiment = experiment

        self.dataset = experiment.args["dataset"]
    ...
    def __read_data__(self):
        ...
        if self.dataset == "ETTh1":
            ...
            border2s = [
                12 * 30 * 24,
                12 * 30 * 24 + 4 * 30 * 24,
                12 * 30 * 24 + 8 * 30 * 24 + (self.pred_len - 1),
            ]
        ...
        zeros_rows = pd.DataFrame(
            np.zeros(((self.pred_len - 1), len(df_raw.columns))), columns=df_raw.columns
        )
        ...
        if self.normalization == "z-score":
            ...
            data_o = np.vstack(
                [
                    (df_data.iloc[: -(self.pred_len - 1) or None]),
                    df_data.iloc[-(self.pred_len - 1) or None :],
                ]
            )
            data = np.vstack(
                [
                    self.scaler.transform(df_data.iloc[: -(self.pred_len - 1) or None]),
                    df_data.iloc[-(self.pred_len - 1) or None :],
                ]
            )
            self.data = self.scaler.transform(
                df_data.iloc[: -(self.pred_len - 1) or None]
            )
            self.data = pd.DataFrame(self.data, columns=train_data.columns)
            self.sample_mean_normalized = (
                self.data[border1s[0] : border2s[0]].mean().to_frame().T
            )
        elif self.normalization == "log":
            if self.features == "S":
                self.shift_values = {
                    df_raw.columns[df_raw.columns.get_loc(self.target)]: (
                        abs(df_raw[self.target].min())
                        if df_raw[self.target].min() < 0
                        else 0
                    )
                }
            else:
                self.shift_values = {
                    col: abs(df_raw[col].min()) if df_raw[col].min() < 0 else 0
            ...
            data = np.vstack(
                [
                    np.log1p(df_data.iloc[: -(self.pred_len - 1) or None]),
                    df_data.iloc[-(self.pred_len - 1) or None :],
                ]
            )
        ...
        if self.features == "M":
            ...
            self.sample_mean = self.sample_mean.iloc[
                :, self.columns.get_loc(self.target)
            ]
        ...
        df_stamp["date"] = pd.to_datetime(
            df_stamp.date, format="%m/%d/%Y %H:%M", errors="coerce"
        )
        if self.timeenc == 0:
    ...
    def __getitem__(self, index):
        ...
        indices_x = self.indices_x[s_begin:s_end]
        indices_y = self.indices_y[r_begin:r_end]
        ...
        return indices_x, indices_y, seq_x, seq_y, seq_x_mark, seq_y_mark, cycle_index
