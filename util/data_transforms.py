...
def transform_data(self):
    ...
    if self.normalization == "z-score":
        ...
        for idx, (scaler_name, scaler) in enumerate(self.scalers.items()):
            mean = scaler.mean_
            std = scaler.scale_
            # print('-------------------check function for "S".===========================')
            print(
                f"{self.columns[idx]} Train Mean: {mean[0]:.3f}, Train Standard Deviation: {std[0]:.3f}"
            )
        ...
        if self.features == "s":
            self.shift_values = {
                self.data.columns[self.data.columns.get_loc(self.target.lower())]: (
                    abs(self.data[self.target.lower()].min())
                    if self.data[self.target.lower()].min() < 0
                    else 0
                )
            }
        else:
            self.shift_values = {
                col: abs(self.data[col].min()) if self.data[col].min() < 0 else 0
